...
fn load_assets_from_path(path: &Path) -> TokenStream {
    let asset_path = path
        .canonicalize()
...
pub fn load_assets(input: TokenStream) -> TokenStream {
    ...
    load_assets_from_path(&asset_path)
}
...
pub fn load_assets_from_out_dir(input: TokenStream) -> TokenStream {
    ...
    let asset_path = Path::new(&env::var("OUT_DIR").expect("OUT_DIR environment variable not set"))
        .join(asset_path);
